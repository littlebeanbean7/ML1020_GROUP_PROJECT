...
class Classifier:
    ...
    def eval(self, model, df, img_folder, classifier_template, batch_size=32):
        validation_steps = math.ceil(df.shape[0] / batch_size)
        validation_generator = ImageDataGenerator(rescale=1. / 255).flow_from_dataframe(
            df,
            directory=img_folder, x_col="img", y_col="classname",
            target_size=(classifier_template.img_width, classifier_template.img_height),
            batch_size=batch_size,
            shuffle=True,
            class_mode='categorical')
    ...
    def cross_validate_eval(self, saved_folder, classifier_template):
        ...
        for i, (train_index, val_index) in enumerate(skf.split(df, y)):
            ...
            res_train = self.eval(model, df_train, "../data/imgs/train/", classifier_template)
            res_val = self.eval(model, df_val, "../data/imgs/train/", classifier_template)
    ...
    def whole_dataset_eval(self, saved_folder, classifier_template):
        ...
        model = load_model(model_save_file)
        res = self.eval(model, df, "../data/imgs/train/", classifier_template)
...
def main():
    clf = Classifier()
    clf.cross_validate_fit(DenseNetTemplate(), "../saved_models")
    clf.cross_validate_eval("../saved_models", DenseNetTemplate())
    #clf.fit_on_wholedataset(DenseNetTemplate(), "../saved_models")
    #clf.whole_dataset_eval("../saved_models", DenseNetTemplate())
